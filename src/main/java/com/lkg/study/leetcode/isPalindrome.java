...
public class isPalindrome {
    ...
    public static void main(String[] args) {
        System.out.println(isPalindrome3(131));
    }
    //todo 反转后数字越界，超出int范围
...
我们将原始数字除以 10，然后给反转后的数字乘上 10，所以，当原始数字小于反转后的数字时，就意味着我们已经处理了一半位数的数字。
    ...
    private static boolean isPalindrome2(int x){
        if (x<0 || (x % 10 == 0&& x !=0)) return false; //（x % 10 == 0&& x !=0）解决x=10（10的倍数，必定返回ture）的问题
        int rev = 0;
        while (x > rev){
            int pop = x % 10;
            x = x / 10;
            if (x == rev) return true; // 总位数为奇数,如果x=10（10的倍数，必定返回ture）
            rev = rev * 10 + pop;
            if (x == rev) return true; // 总位数为偶数
        }
        return true;
    }
    ...
    private static boolean isPalindrome3(int x){
        ...
        //  总位数为偶数 || 总位数为奇数      如果x=10（10的倍数，必定返回ture）
